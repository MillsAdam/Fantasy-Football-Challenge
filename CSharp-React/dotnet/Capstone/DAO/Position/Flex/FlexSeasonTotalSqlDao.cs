    ...
    public class FlexSeasonTotalSqlDao : IFlexSeasonTotalDao
    {
        ...
        private PlayerStatsExtDto MapRowToFlexStat(NpgsqlDataReader reader)
        {
            return new PlayerStatsExtDto()
            {
                ...
                RushingAttempts = Convert.ToInt32(reader["rushing_attempts"]),
                RushingYards = Convert.ToInt32(reader["rushing_yards"]),
                RushingYardsPerAttempt = Convert.ToDouble(reader["rushing_yards_per_attempt"]),
                RushingTouchdowns = Convert.ToInt32(reader["rushing_touchdowns"]),
                ReceivingTargets = Convert.ToInt32(reader["receiving_targets"]),
                Receptions = Convert.ToInt32(reader["receptions"]),
                ReceivingYards = Convert.ToInt32(reader["receiving_yards"]),
                ReceivingYardsPerReception = Convert.ToDouble(reader["receiving_yards_per_reception"]),
                ReceivingTouchdowns = Convert.ToInt32(reader["receiving_touchdowns"]),
                ReturnTouchdowns = Convert.ToInt32(reader["return_touchdowns"]),
                TwoPointConversions = Convert.ToInt32(reader["two_point_conversions"]),
                Usage = Convert.ToDouble(reader["usage"]),
                FumblesLost = Convert.ToInt32(reader["fumbles_lost"]),
